        ...
        @foreach($applications as $application)
            <tr>
                ...
                <td>
                    ...
                    @else
                        <span class="badge rounded-pill bg-warning text-white"> IN-ACTIVE </span>
                    @endif
                </td>
                ...
                <td>
                    ...
                    @if($application->status === \App\Data\FeatureApplicationStatus::OFF)
                        <span class="badge rounded-pill bg-secondary text-white"> {{$application->status}} </span>
                    @elseif($application->status === \App\Data\FeatureApplicationStatus::ON)
                        <span
                            class="badge rounded-pill bg-primary text-white"> {{$application->status}} </span>
                    @elseif($application->status === \App\Data\FeatureApplicationStatus::LAUNCHED)
                        <span
                            class="badge rounded-pill bg-success text-white"> {{$application->status}} </span>
                    @elseif($application->status === \App\Data\FeatureApplicationStatus::PAUSED)
                        <span
                            class="badge rounded-pill bg-warning text-white"> {{$application->status}} </span>
                    @endif
                ...
                <td>
                    <x-link
                        ...
                        <i class="tio-update"></i> Modify allocations
                    </x-link>
                    ...
                    <x-link
                        ...
                        <i class="tio-toggle-on"></i> Toggle overrides
                    </x-link>
